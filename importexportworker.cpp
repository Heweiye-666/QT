...
bool ImportExportWorker::importFromCSV()
{
    ...
    while (!in.atEnd()) {
        ...
        if (fields.size() >= 5) {
            ...
            QString account = fields[2].trimmed();  // 新增：账号字段
            ...
            if (!website.isEmpty() && !username.isEmpty()) {
                ...
                Database::instance().addPassword(website, username, encryptedAccount, encryptedPassword, notes);
                importedCount++;
...
bool ImportExportWorker::exportToCSV()
{
    ...
    QString exportType = m_exportEncrypted ? "保密版" : "未保密版";
    emit progressChanged(100, QString("导出(%1)完成，共导出 %2 条记录").arg(exportType).arg(exportedCount));
